...
function bind_one(key, value) {
    return value === null ? null : typemap[key].bind(value);
}
...
function unbind(types, row) {
    return sql.unbind(types, row, unbind_one);
}
export default class SQLite {
    ...
    async create(as, record) {
        ...
        if (!common.BIGINT_STRING_TYPES.includes(type) && type !== "string") {
            ...
            const rows = this.#sql(query).all(await bind(as.types, record));
    ...
    async #count(as, where) {
        ...
        const rows = this.#sql(query).all(binds);
        assert.true(rows.length === 1, `COUNT(*) must return 1 row (got ${rows.length})`);
        const n = rows[0].n;
        assert.bigint(n, `COUNT returned non-bigint: ${typeof n}`);
        if (n > BigInt(Number.MAX_SAFE_INTEGER))
            throw E.count_overflow(table, n);
        return Number(n);
    }
