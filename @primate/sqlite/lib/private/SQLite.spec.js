...
const users_types = { id: "u32", name: "string", age: "u8" };
const users_as = { table: "users", pk: "id", types: users_types };
const posts_types = { id: "u32", user_id: "u32", title: "string" };
const posts_as = { table: "posts", pk: "id", types: posts_types };
const tx_types = { id: "u64", amount: "u128", memo: "string" };
...
query_equals("select all", () => db.read(users_as, { where: {} }), "SELECT * FROM `users`");
query_equals("select with fields", () => db.read(users_as, { where: {}, fields: ["name", "age"] }), "SELECT `name`, `age` FROM `users`");
query_equals("where equality", () => db.read(users_as, { where: { name: "bob" } }), "SELECT * FROM `users` WHERE `name` = $name");
query_equals("where multiple fields", () => db.read(users_as, { where: { name: "bob", age: 25 } }), "SELECT * FROM `users` WHERE `name` = $name AND `age` = $age");
query_equals("where null", () => db.read(users_as, { where: { name: null } }), "SELECT * FROM `users` WHERE `name` IS NULL");
query_equals("where $ne", () => db.read(users_as, { where: { age: { $ne: 30 } } }), "SELECT * FROM `users` WHERE `age` != $age__ne");
query_equals("where $gt", () => db.read(users_as, { where: { age: { $gt: 18 } } }), "SELECT * FROM `users` WHERE `age` > $age__gt");
query_equals("where $gte + $lt", () => db.read(users_as, { where: { age: { $gte: 18, $lt: 65 } } }), "SELECT * FROM `users` WHERE `age` >= $age__gte AND `age` < $age__lt");
query_equals("sort asc", () => db.read(users_as, { where: {}, sort: { name: "asc" } }), "SELECT * FROM `users` ORDER BY `name` ASC");
query_equals("sort desc", () => db.read(users_as, { where: {}, sort: { age: "desc" } }), "SELECT * FROM `users` ORDER BY `age` DESC");
query_equals("limit", () => db.read(users_as, { where: {}, limit: 10 }), "SELECT * FROM `users` LIMIT 10");
query_equals("where + sort + limit", () => db.read(users_as, {
    where: { age: { $gt: 21 } },
    sort: { name: "asc" },
    limit: 5,
