...
test.case("pass", assert => {
    assert(string).type();
    assert(string.parse("test")).equals("test").type();
});
test.case("default", assert => {
    [string.default("foo"), string.default(() => "foo")].forEach(d => {
        assert(d).type();
        assert(d.parse(undefined)).equals("foo").type();
        assert(d.parse("foo")).equals("foo").type();
        assert(d.parse("bar")).equals("bar").type();
...
test.case("validator - startsWith", assert => {
    ...
    assert(sw).type();
    assert(sw.parse("/")).equals("/").type();
    assert(sw.parse("/foo")).equals("/foo").type();
    assert(() => sw.parse("foo")).throws("\"foo\" does not start with \"/\"");
});
test.case("validator - endsWith", assert => {
    const ew = string.endsWith("/");
    assert(ew).type();
    assert(ew.parse("/")).equals("/").type();
    assert(ew.parse("foo/")).equals("foo/").type();
...
test.case("validator - min", assert => {
    ...
    assert(min.parse("hello")).equals("hello").type();
    assert(min.parse("universe")).equals("universe").type();
...
test.case("validator - max", assert => {
    ...
    assert(max.parse("hello")).equals("hello").type();
    assert(max.parse("hi")).equals("hi").type();
...
test.case("validator - length", assert => {
    ...
    assert(length.parse("hello")).equals("hello").type();
    assert(length.parse("hi")).equals("hi").type();
...
test.case("combined validators", assert => {
    ...
    assert(sew).type();
    assert(sew.parse("/.")).equals("/.").type();
    assert(sew.parse("/foo.")).equals("/foo.").type();
    {
